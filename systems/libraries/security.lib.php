...
class security
{
    ...
    /**
     * set token for a form or a request
     *
     * @param string $name
    ...
    public function check_token($name, $method = 'POST')
    {
        ...
        $ss_name = 'session_' . $name;

        if (empty($_SESSION[$ss_name]) || empty($check)) {
            return false;
        }

        $ss = $_SESSION[$ss_name];

        if (!empty($check) && !empty($ss)) {
            if ($check == $ss) {
                return TRUE;
            }
    ...
    function isValid($name, $value ) {
        //set name for session
        $ss_name = 'session_' . $name;
        $ss = isset($_SESSION[$ss_name]) ? $_SESSION[$ss_name] : '';

        if (!empty($value) && isset($ss)) {
            if ($value == $ss) {
                return TRUE;
            }
            return FALSE;
     ...
     * @return    string
     */
    public function cleanXSS($str, $is_image = FALSE)
    {
        /*
