...
ZenView::block('Cấu hình blog', function() use ($registry) {
    ZenView::padded(function() use ($registry) {
        ...
        $list_id = tplConfig('list_blog_cat_display', 'zencms-mobile-web');
        ...
        foreach($list_folders as $id=>$folder) {
            echo '<option value="' . $id . '" ' . ($id==tplConfig('id_box_event', 'zencms-mobile-web') ? 'selected':'') . '>' . $folder . '</option>';
        }
        echo '</select></div></div>';
        ...
        <div class="col-lg-9"><input type="text" id="num_post_per_box" name="num_post_per_box" value="' . tplConfig('num_post_per_box', 'zencms-mobile-web') . '" class="form-control"/></div>
        </div>';
        /*
        echo '<div class="form-group">
        <label for="id_post_hot" class="col-lg-2 control-label">ID bài viết hot nhất</label>
        <div class="col-lg-9"><input type="text" id="id_post_hot" name="id_post_hot" value="' . implode(',', tplConfig('id_post_hot', 'zencms-mobile-web')) . '" class="form-control"/>
        ...
        <div class="col-lg-9"><input type="text" id="num_post_top_hot" name="num_post_top_hot" value="' .  tplConfig('num_post_top_hot', 'zencms-mobile-web') . '" class="form-control"/></div>
        </div>';
        */
        echo '<div class="form-group">
        <label for="num_post_in_folder" class="col-lg-2 control-label">Số lượng bài viết hiển thị trong thư mục</label>
        <div class="col-lg-9"><input type="text" id="num_post_in_folder" name="num_post_in_folder" value="' .  tplConfig('num_post_in_folder', 'zencms-mobile-web') . '" class="form-control"/></div>
        </div>';
        echo '<div class="form-group">
        <label for="num_rand_post_in_folder" class="col-lg-2 control-label">Số bài viết ngẫu nhiên trong thư mục</label>
        <div class="col-lg-9"><input type="text" id="num_rand_post_in_folder" name="num_rand_post_in_folder" value="' .  tplConfig('num_rand_post_in_folder', 'zencms-mobile-web') . '" class="form-control"/></div>
        </div>';
        echo '<div class="form-group">
        <label for="num_same_post_in_post" class="col-lg-2 control-label">Số bài viết tương tự trong bài viết</label>
        <div class="col-lg-9"><input type="text" id="num_same_post_in_post" name="num_same_post_in_post" value="' .  tplConfig('num_same_post_in_post', 'zencms-mobile-web') . '" class="form-control"/></div>
        </div>';
        echo '<div class="form-group">
        <label for="num_rand_post_in_post" class="col-lg-2 control-label">Số bài viết ngẫu nhiên trong bài viết</label>
        <div class="col-lg-9"><input type="text" id="num_rand_post_in_post" name="num_rand_post_in_post" value="' .  tplConfig('num_rand_post_in_post', 'zencms-mobile-web') . '" class="form-control"/></div>
        </div>';
        echo '<div class="form-group">
...
ZenView::block('Cài đặt slider', function() {
   ZenView::padded(function() {
       ...
       echo '<form role="form" class="form-horizontal validatable" method="POST">';
       $slider_config = tplConfig('slider_config', 'zencms-mobile-web');
